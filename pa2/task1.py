...
def select_kth_smallest_1(k, S):
    ...
    if n == 1:
        ...
        if len(S1) >= k:
            return select_kth_smallest_1(k, S1)
        elif len(S1) + len(S2) >= k:
            return m
        else:
            return select_kth_smallest_1(k - len(S1) - len(S2), S3)

def select_kth_smallest_2(k, S):
    n = len(S)
    if n < 50:
        ...
        m = select_kth_smallest_2(len(medians) // 2, medians)
        ...
        if len(S1) >= k:
            return select_kth_smallest_2(k, S1)
        elif len(S1) + len(S2) >= k:
            return m
        else:
            return select_kth_smallest_2(k - len(S1) - len(S2), S3)      
...
def main():
    ...
    result1 = select_kth_smallest_1(k, S)
    stop_time = time.time()
    ...
    start_time = time.time()
    result2 = select_kth_smallest_2(k, S)
    stop_time = time.time()
